...
Step-2: Go to newly created Azure Cosmos DB account blade --. Click on Keys --> Note down Cosmos DB URL, Primarykey fields
Step-3; Open OMS.sln file in VStudio --> Go to OMS.Proudcts Project --> open ProcuctContrller.cs --> remplace below two properties with URL, Key found in Step-2. 
...
Step-4: In Vstudio --> Right click on the project OMS.PRoudcts --> and click on "Set as start project" --> click on F5 or Run --> Start debugging --> It will open swagger page
Step-5: You can post the documents from Swagger with the JSON format as mentioned below so code will connect to COSMOSDB and isert document. Use get method in Swagger to fetch all doucments.
